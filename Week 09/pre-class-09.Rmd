...
```{r}
#The URLs are in the form "https://money.cnn.com/quote/quote.html?symb=" followed by the 4 letter stock identifying abbreviation. These names are the first 4 letters in the first column of the Whats_Moving table. The urls are collected by pasting together the base URL with the 4 letter symbol by using substr to extract the abbreviations.
urls <- c()
names <- Whats_Moving[,1]
for(i in seq_along(names)){
  urls <- c(urls, paste(paste("https://money.cnn.com/quote/quote.html?symb=", substr(names[i], 1, 4), sep = "")))
}
urls
```
